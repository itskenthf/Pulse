import { ensureWidgetRegistered, getCurrentBook } from "@pulse/database";
 ...
 * user's own `reading` row. The scheduler still calls this on the normal
...
export async function fetchReadingData(context: WidgetFetchContext): Promise<ReadingData> {
  ...
  const book = await getCurrentBook(context.userId);

  return { book, fetchedAt: new Date().toISOString() };
}
